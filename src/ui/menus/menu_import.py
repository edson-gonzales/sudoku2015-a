...
def display_import_menu(display_main_menu):
    ...
    menu.clear_items()
    menu.add_item((1, 'Import from CSV File', import_from_csv_file, display_main_menu))
    menu.add_item((2, 'Import from TXT File', import_from_txt_file, display_main_menu))
    menu.add_item((3, 'Import from Input', import_from_input, display_main_menu))
    menu.add_item((4, 'Back', display_main_menu, 0))
    menu.add_item((0, 'Exit', None))
    menu.ask()

def import_from_csv_file(display_main_menu):
    """
    Imports a game from a file in csv format (i.e. 123456789,123456789,123...)
    display_main_menu -- is the main menu object that will be eventually called.
    """
    file_types = [('CSV files', '*.csv'), ('All files', '*')]
    file_to_be_imported = file_browser_dialog(file_types)
    if file_to_be_imported.file_exists():
        store_last_used_folder(file_to_be_imported.file_path)
        file_content = file_to_be_imported.read_content()
        imported_game = converter.convert_csv_string_to_game_list(file_content, blank_character)
        print imported_game
    else:
        display_import_menu(display_main_menu)


def import_from_txt_file(display_main_menu):
    """
    ...
    if file_to_be_imported.file_exists():
        store_last_used_folder(file_to_be_imported.file_path)
        file_content = file_to_be_imported.read_content()
        imported_game = converter.convert_txt_string_to_game_list(file_content, blank_character)
        print imported_game
    else:
        display_import_menu(display_main_menu)


def import_from_input(display_main_menu):
    """
    ...
    if len(input_content) == 81:
        imported_game = converter.convert_input_string_to_game_list(input_content, blank_character)
        print imported_game
    else:
        display_import_menu(display_main_menu)


def file_browser_dialog(file_types):
    """
    ...
    root = Tkinter.Tk()
    root.withdraw()
    folder = configuration.file_path_save
...
def store_last_used_folder(last_used_file_path):
    """
    ...
    if os.path.exists(folder_path):
        configuration.file_path_save = folder_path
